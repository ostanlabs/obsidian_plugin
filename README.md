...
1. `add_vault {"path": "/Users/you/Obsidian/Projects/MyProject", "bootstrap": "auto"}`
   ...
   entity type, `archive/`, `workspaces.json`, a default canvas named after the
   vault, and the bundled Obsidian plugin **plus its Dataview dependency**
   installed into `.obsidian/plugins/` and pre-enabled — pass
   `installPlugin: false` for a headless vault; Dataview install needs network
   and degrades to a warning when offline); an existing
