...
private registerCommands(): void {
		...
		const modalOptions: StructuredItemModalOptions = {
			...
			defaultTitle: "New Item",
				...
				const modal = new StructuredItemModal(
					...
					modalOptions,
					async (result) => {
						// Map StructuredItemResult to ItemCreationResult
						const itemResult: ItemCreationResult = {
							type: result.type,
	...
	item
		...
		.onClick(async () => {
			console.log("[Canvas Plugin] 'Create Task' menu item clicked", {
				canvasPath: canvasFile.path,
			});
			const modalOptions: StructuredItemModalOptions = {
				showTitleInput: true,
		...
		item
			...
			const modalOptions: StructuredItemModalOptions = {
				...
				showTemplateSelector: true,
				typeEditable: false,
				fixedType: "accomplishment",
				modalTitle: "Create Accomplishment",
				submitButtonText: "Create",
				defaultTitle: "New Accomplishment",
			};
				const modal = new StructuredItemModal(
					this.app,
					this.settings,
					...
					async (result) => {
						// Map StructuredItemResult to ItemCreationResult
						const itemResult: ItemCreationResult = {
							type: "accomplishment", // Force accomplishment type
							effort: result.effort,
